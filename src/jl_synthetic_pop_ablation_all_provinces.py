...
if __name__ == "__main__":
    ...
    for prov in data["prov_abbrv"].sort_values().unique():
        ...
        try:
            ...
            df_prov_full = df_prov_full[[u for u in jl_vae.cols + ["log_price"] if u not in ["prov_abbrv"]] + ["x_norm", "y_norm"]].assign(flag_air_conditioning_Missing = lambda x: x["flag_air_conditioning"] == "Missing",
            ...
            for j,df_vae in enumerate([df_prov_full.copy(), df_prov_dropna.copy()]):
                ...
                df_vae[["y_latent", "x_latent"]] = df_vae[["x_norm", "y_norm"]] / 2 + 0.5
                ...
                path_vae = jl_vae.path_pop_synth + f"ablation_vae/vae_{['full', 'dropna'][j]}_{date}.pkl"
                path_settings = jl_vae.path_pop_synth + f"ablation_vae/settings_{['full', 'dropna'][j]}_{date}.pkl"
                
                vae.save(path_vae)
                vae.save_settings(path_settings)
                
                df_sample = vae.get_sample_from_vae(nf_dict = None)
                ...
                df_sample.to_csv(jl_vae.path_pop_synth + f"pop_samples_ablation/synthetic_pop_{['full', 'dropna'][j]}_{date}.csv")
