...
if __name__ == "__main__":
    ...
    for prov in data["prov_abbrv"].sort_values().unique():
        ...
        try:
            ...
            path_nf = path_pop_synth95 + f"nf_models/nf_{date}.pkl"
            ...
            df_prov_full95 = df_prov_full.sample(frac = 0.95, random_state = 1111)

            print(prov, "NF")
            t0 = time()
            
            nf_dict = nfg.train_model(df_prov_full95[["x_norm", "y_norm"]], 
            ...
            for j,df_vae in enumerate([df_prov_full95.copy(), df_prov_dropna.copy()]):
                if j:
                    continue
                print(prov, j, "VAE")
                date = "250703" + prov
                ...
                path_vae = path_pop_synth95 + f"vae_models/vae_{['full', 'dropna'][j]}_{date}.pkl"
                path_settings = path_pop_synth95 + f"vae_models/settings_{['full', 'dropna'][j]}_{date}.pkl"
                ...
                df_sample.to_csv(path_pop_synth95 + f"pop_samples/synthetic_pop_{['full', 'dropna'][j]}_{date}.csv")
